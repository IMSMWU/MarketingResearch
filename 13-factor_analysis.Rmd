...
spotify_data <- na.omit(spotify_data[,c(1,16:26)])
nsongs <- length(unique(spotify_data$id))
...
We have collected data for `r nsongs` songs and stored the results in the data set "spotify_data".
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=TRUE}
pc2 <- principal(spotify_data, nfactors = nfac, rotate = "none")
pc2
```
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=TRUE}
pc3 <- principal(spotify_data, nfactors = nfac, rotate = "varimax")
pc3
```
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=TRUE}
pc4 <- principal(spotify_data, nfactors = nfac, rotate = "oblimin", scores = TRUE)
...
```{r message=FALSE, warning=FALSE, eval=TRUE, echo=TRUE}
energetic <- spotify_data[,c(names(rc1))]
